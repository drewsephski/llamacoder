...
description: >
  ...
  are identical and sacred — the white canvas, a single saturated Squid blue (#326DF5) as the only
  accent, the three-step gray text ladder, Aeonik (display/numerals/chrome) + Aeonik (body),
...
colors:
  ...
  text-muted: "#0062FF"
  text-light: "#9A9A9A"
  accent-light: "rgba(50, 109, 245, 0.08)"
  accent-medium: "rgba(50, 109, 245, 0.15)"
  border: "rgba(50, 109, 245, 0.2)"
  card-bg: "rgba(50, 109, 245, 0.04)"
  positive: "#059669"
  negative: "#dc2626"
...
typography:
  # — reading ramp (Aeonik body + Aeonik chrome) —
  body:    { fontFamily: "Aeonik", cqw: 0.85, weight: 400, lineHeight: 1.6, color: "text-muted" }
  h4-eyebrow:{ fontFamily: "Aeonik", cqw: 0.8, weight: 500, tracking: "0.08em", upper: true, color: "primary" }
  tag:     { fontFamily: "Aeonik", px: 12, weight: 500, color: "primary" }
  counter: { fontFamily: "Aeonik Mono", px: 13, weight: 400, tracking: "0.05em", color: "text-muted" }
  # — display / numerical ramp (Aeonik, near-black headings / blue numerals) —
  h3:      { fontFamily: "Aeonik", cqw: 1.25, weight: 500, lineHeight: 1.3, tracking: "-0.02em", color: "text" }
  stat-num:{ fontFamily: "Aeonik", cqw: 1.9, weight: 700, lineHeight: 1.0, color: "primary" }
  blockquote:{ fontFamily: "Aeonik", cqw: 2.4, weight: 500, lineHeight: 1.35, color: "text" }
  h2:      { fontFamily: "Aeonik", cqw: 2.6, weight: 500, lineHeight: 1.1, tracking: "-0.02em", color: "text" }
  metric-value:{ fontFamily: "Aeonik", cqw: 3.0, weight: 700, lineHeight: 1.0, color: "primary" }
  h1:      { fontFamily: "Aeonik", cqw: 4.2, weight: 700, lineHeight: 1.08, tracking: "-0.02em", color: "text" }
  quote-mark:{ fontFamily: "Aeonik", cqw: 8.0, weight: 700, lineHeight: 0.5, color: "primary", opacity: 0.15 }
...
- **Fonts** — already set to **Aeonik** (display/body) / **Aeonik Mono** (metadata); ignore any preset font name lingering in prose.
- **Weights** — the brand font ships `{400, 500, 600, 650, 700, 800, 900}` only; every weight is clamped to these — ignore higher preset weights (e.g. 600/700) in prose.
...
The voice is two faces in fixed roles: **DM Sans** (display, every numeral, all chrome —
eyebrows uppercase 0.08em) and **DM Sans** (body, muted gray, line 1.6). Headlines are near-black;
...
- **DM Sans** (display/numerals/chrome) + **DM Sans** (body) — near-black headlines, cobalt numerals.
...
Two ramps. The **reading ramp** (DM Sans body 0.85cqw muted; DM Sans eyebrow uppercase 0.08em
cobalt) carries copy + chrome; the **display/numerical ramp** (DM Sans `h3` 1.25cqw → `h1`
...
- **Headlines near-black, −0.02em**; **eyebrows cobalt, uppercase, 0.08em**; **numerals cobalt 600–700**; **body DM Sans 400 muted, line 1.6**. No italic, no uppercase body, no cobalt headline.
...
**Composes** accent-line, meta, h1, body sub. **Focal** a 2-line DM Sans `h1` near-black, left,
...
**Focal** a row of tinted cards — cobalt `metric-value` + DM Sans label + muted desc + optional
...
blockquote, cite. **Focal** a DM Sans `blockquote` near-black under a 15%-opacity cobalt
...
DM Sans body column beside a cobalt-8% highlight block (4px cobalt left rule) carrying an inline pull
...
a DM Sans `h1` near-black, centered, with the one solid cobalt `cta-button` pill below. **Accent**
...
- Use **tinted cards** (4% fill, 20% border, 10–14px radius, no shadow); body DM Sans 400 muted, line 1.6.
...
- **Type** — DM Sans headings −0.02em near-black, cobalt eyebrows 0.08em + numerals; DM Sans body muted line 1.6; ≥1.4cqw floor.
...
- **DM Sans + DM Sans via Google Fonts.** CJK pairing (Noto Sans SC 700 display / Noto Serif SC 400 body) carries over; the eyebrow's uppercase+tracking signal weakens in CJK — pair it with the accent-line.
...
```html
<style>
@font-face{font-family:"Aeonik";font-weight:700;font-style:normal;font-display:block;src:url("assets/fonts/Aeonik-Bold.ttf") format("truetype");}
@font-face{font-family:"Aeonik";font-weight:500;font-style:normal;font-display:block;src:url("assets/fonts/Aeonik-Medium.ttf") format("truetype");}
@font-face{font-family:"Aeonik";font-weight:400;font-style:normal;font-display:block;src:url("assets/fonts/Aeonik-Regular.ttf") format("truetype");}
@font-face{font-family:"Aeonik Mono";font-weight:400;font-style:normal;font-display:block;src:url("assets/fonts/AeonikMono-Regular.otf") format("opentype");}
@font-face{font-family:"Instrument Serif";font-weight:400;font-style:normal;font-display:block;src:url("assets/fonts/InstrumentSerif-Regular.woff") format("woff");}
...
@font-face{font-family:"Inter";font-weight:400;font-style:normal;font-display:block;src:url("assets/fonts/Inter-Regular.woff") format("woff");}
