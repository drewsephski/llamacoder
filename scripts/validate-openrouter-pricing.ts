...
for (const activeModel of MODELS) {
  ...
  if (!liveModel) {
    failures.push(`${activeModel.value}: missing from the live model catalog`);
    continue;
  }
...
if (failures.length > 0) {
  throw new Error(`OpenRouter pricing validation failed:\n- ${failures.join("\n- ")}`);
}

console.log(`Validated pricing for ${MODELS.length} active OpenRouter models.`);
