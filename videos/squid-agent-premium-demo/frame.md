...
description: >
  ...
  DM Sans in lowercase weight 900 treated as graphic primitive, IBM Plex Mono chrome (uppercase,
...
typography:
  # — reading ramp —
  body:    { fontFamily: "DM Sans", cqw: 1.2, weight: 400, lineHeight: 1.6 }
  lead:    { fontFamily: "DM Sans", cqw: 1.6, weight: 400, lineHeight: 1.5 }
  caption: { fontFamily: "DM Sans", cqw: 0.9, weight: 400, lineHeight: 1.5 }
  label:   { fontFamily: "DM Mono", cqw: 0.72, weight: 400, tracking: "0.14em", upper: true }
  # — display / hero ramp (DM Sans, lowercase, negative tracking) —
  h3:      { fontFamily: "DM Sans", cqw: 2.8, weight: 600, lineHeight: 1.2, lower: true }
  quote-text:{ fontFamily: "DM Sans", cqw: 3.8, weight: 700, lineHeight: 1.15, tracking: "-0.02em", lower: true }
  h2:      { fontFamily: "DM Sans", cqw: 4.5, weight: 700, lineHeight: 1.1, tracking: "-0.02em", lower: true }
  stat-value:{ fontFamily: "DM Sans", cqw: 5.5, weight: 900, lineHeight: 1.0, tracking: "-0.04em" }
  h1:      { fontFamily: "DM Sans", cqw: 7.5, weight: 800, lineHeight: 0.9, tracking: "-0.03em", lower: true }
  fadelist-item:{ fontFamily: "DM Sans", cqw: 7.5, weight: 900, lineHeight: 1.0, tracking: "-0.03em", lower: true }
  quote-mark:{ fontFamily: "DM Sans", cqw: 10.0, weight: 900, lineHeight: 0.6 }
  fadelist-title:{ fontFamily: "DM Sans", cqw: 10.5, weight: 900, lineHeight: 0.9, tracking: "-0.04em", lower: true }
  display: { fontFamily: "DM Sans", cqw: 13.0, weight: 900, lineHeight: 0.88, tracking: "-0.04em", lower: true }
...
- **Fonts** — already set to **DM Sans** (display) / **Arial** (body); ignore any preset font name lingering in prose.
...
text and becomes graphic primitive.** DM Sans `display` at 13cqw puts a single lowercase word
...
**DM Sans** carries every text role from display to body — expressive range from weight (400–900)
...
- **Massive lowercase DM Sans 900**, negative-tracked, as graphic primitive (display 13cqw).
...
Two ramps. The **reading ramp** (DM Sans body 1.2cqw, mono label 0.72cqw) carries copy + chrome; the
**display ramp** (DM Sans `h2` 4.5cqw → `display` 13cqw, weight 700–900) carries every statement.
...
- **DM Sans display is lowercase, weight 700–900, negative-tracked** (−0.04em largest, −0.02em h2). **Mono chrome is uppercase, 0.1em+.** No italic, no underline, no uppercase display.
...
DM Sans `display` (13cqw) lowercase in ink, left-anchored, over a small ink rule stub + mono kicker; a
DM Sans lead line beneath in 75% ink. **Chrome** mono catalogue number top-left, mono meta top-right
...
**Ground** ink-black. **Composes** kicker, display. **Focal** a 2–4 word DM Sans `display`/`h1`
...
row of three top-border-only stat-cards — big DM Sans-900 numeral in `{colors.fire-orange}`, DM Sans
...
**Ground** ink-black. **Composes** fadelist (3 stacked DM Sans-900 words at 1.0/0.5/0.22), fadelist-title.
...
DM Sans `quote-text` (700, lowercase) at ≤78cqw under an oversized fire-orange `quote-mark` (10cqw,
...
- Set every DM Sans display in **lowercase weight 900**, negative-tracked — the system's signature.
...
- Never uppercase DM Sans display; never add a second accent color.
...
- No serif companion; chrome is never DM Sans.
...
- **Type** — DM Sans lowercase 900 negative-tracked, fit-to-measure; mono chrome uppercase 0.14em; ≥1.4cqw floor.
...
- **DM Sans + IBM Plex Mono via Google Fonts**; Noto Sans SC is the CJK fallback (the lowercase-display signal has no CJK equivalent — the two-register color system carries the identity, per the source).
