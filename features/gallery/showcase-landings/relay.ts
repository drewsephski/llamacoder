...
/* Hallmark · macrostructure: Workbench · genre: modern-minimal · theme: Cobalt · tone: technical · anchor hue: electric cobalt · nav: N1b · footer: Ft2 · enrichment: E4 CSS product surfaces · contrast: pass (40–41) · slop: pass (42–49) · mobile: pass (34, 49–57) */
:root {
...
const files = [
  { name: "checkout.ts", state: "Changed", detail: "Payment confirmation path" },
  { name: "route.test.ts", state: "Passed", detail: "Regression coverage" },
  { name: "release.yml", state: "Changed", detail: "Deployment policy" },
];
...
export default function App() {
  ...
  const [selectedFile, setSelectedFile] = useState("checkout.ts");
  ...
  const runChecks = () => {
    ...
    window.setTimeout(() => setCheckState("success"), 900);
  };
  ...
  return (
    <main className="relay-shell">
        ...
        <figure className="release-surface" id="evidence" aria-label="Relay release evidence workspace">
          ...
          <div className="surface-grid">
            <aside className="file-list" aria-label="Changed files">
              <p>CHANGE SET</p>
              {files.map((file) => (
                <button key={file.name} type="button" className={selectedFile === file.name ? "file-row is-active" : "file-row"} onClick={() => setSelectedFile(file.name)}>
                  <span>{file.name}</span><small>{file.state}</small>
                </button>
              ))}
            </aside>
            <div className="diff-view">
              <div className="diff-head"><span>{selectedFile}</span><span>READY FOR REVIEW</span></div>
              <div className="code-line"><i>−</i><code>return redirect('/complete')</code></div>
              <div className="code-line code-line--plus"><i>+</i><code>return verifiedRedirect(order)</code></div>
              <div className="code-line code-line--plus"><i>+</i><code>audit.record('checkout_confirmed')</code></div>
              <div className="evidence-note"><span>WHY IT CHANGED</span><p>The confirmation route now records the same order state used by the receipt view.</p></div>
            </div>
      ...
      <section className="walkthrough" id="checks">
        <div className="section-intro"><p className="machine-label">WORKBENCH / POLICY</p><h2>Read the decision, not a dashboard.</h2></div>
        <div className="walkthrough-grid">
          <article><span>INPUT</span><h3>Change context</h3><p>Files, owners, and the release intent stay attached to the decision.</p></article>
          <article><span>RULE</span><h3>Review policy</h3><p>Each requirement names the evidence it expects and the reason it exists.</p></article>
          <article><span>OUTPUT</span><h3>Release record</h3><p>The final record shows what passed, what changed, and who can revisit it.</p></article>
        </div>
      </section>
      ...
      <section className="verification" id="verification">
        ...
        <div className="verification-log" aria-label="Verification output">
          <div><span>01</span><p>Type contract</p><strong>{checkState === "success" ? "PASS" : "READY"}</strong></div>
          <div><span>02</span><p>Regression coverage</p><strong>{checkState === "success" ? "PASS" : "READY"}</strong></div>
          <div><span>03</span><p>Rollback note</p><strong>{checkState === "success" ? "ATTACHED" : "READY"}</strong></div>
        </div>
      </section>

      <aside className="sticky-cta">
        <p><strong>Relay is a fictional product concept.</strong> Explore the interaction, then build your own version.</p>
      ...
      <div className={paletteOpen ? "command-menu is-open" : "command-menu"} aria-hidden={!paletteOpen}>
        ...
        <section className="command-panel" role="dialog" aria-modal="true" aria-label="Search Relay">
          ...
          <p className="command-group">NAVIGATE</p>
          ...
          <footer><span>↑↓ move</span><span>↵ open</span><span>esc close</span></footer>
        </section>
      </div>
    </main>
  );
...
.nav-center a { display: inline-flex; align-items: center; min-height: 44px; padding: var(--space-sm); color: var(--color-muted); font-size: var(--text-sm); line-height: 1; white-space: nowrap; }
...
.command-trigger { display: flex; align-items: center; gap: var(--space-lg); min-height: 40px; padding: var(--space-xs) var(--space-sm); border: var(--rule-hair) solid var(--color-rule); border-radius: var(--radius-sm); background: var(--color-paper-2); color: var(--color-muted); cursor: pointer; }
kbd { font-family: var(--font-mono); font-size: var(--text-xs); }
.button { display: inline-flex; align-items: center; justify-content: center; min-height: 44px; padding: var(--space-sm) var(--space-md); border: var(--rule-hair) solid var(--color-rule-2); border-radius: var(--radius-sm); cursor: pointer; font-weight: 500; white-space: nowrap; transition: transform var(--dur-short) var(--ease-out), background-color var(--dur-short) var(--ease-out), border-color var(--dur-short) var(--ease-out), color var(--dur-short) var(--ease-out); }
...
.release-surface { width: calc(100% + 8vw); min-width: 0; margin: 0; border: var(--rule-hair) solid var(--color-rule-2); border-radius: var(--radius-md) 0 0 var(--radius-md); background: var(--color-paper); box-shadow: 0 18px 42px var(--color-shadow); overflow: hidden; }
.release-surface > figcaption { display: flex; justify-content: space-between; gap: var(--space-md); padding: var(--space-md); border-bottom: var(--rule-hair) solid var(--color-rule); font-family: var(--font-mono); font-size: var(--text-xs); }
...
.file-list > p, .review-panel > p, .walkthrough article > span, .evidence-note > span { margin: 0 0 var(--space-md); color: var(--color-muted); font-family: var(--font-mono); font-size: 0.66rem; letter-spacing: 0.06em; }
.file-row { display: grid; width: 100%; gap: var(--space-2xs); padding: var(--space-sm); border: 0; border-left: 2px solid transparent; background: transparent; text-align: left; cursor: pointer; }
.file-row span { font-family: var(--font-mono); font-size: var(--text-xs); }
.file-row small { color: var(--color-muted); }
...
.diff-head { display: flex; justify-content: space-between; gap: var(--space-sm); padding-bottom: var(--space-md); border-bottom: var(--rule-hair) solid var(--color-rule); font-family: var(--font-mono); font-size: var(--text-xs); }
.diff-head span:last-child { color: var(--color-success); }
...
.walkthrough article { min-width: 0; padding: var(--space-xl); }
...
.walkthrough article p { margin: var(--space-md) 0 0; color: var(--color-muted); font-size: var(--text-sm); line-height: 1.7; }
...
.verification-copy > p:not(.machine-label) { max-width: 48ch; color: var(--color-graphite-muted); line-height: 1.7; }
...
.verification-log div { display: grid; grid-template-columns: auto minmax(0, 1fr) auto; gap: var(--space-lg); align-items: center; padding: var(--space-lg) 0; border-bottom: var(--rule-hair) solid var(--color-graphite-2); font-family: var(--font-mono); }
.verification-log span { color: var(--color-graphite-muted); }
.verification-log p { margin: 0; }
.verification-log strong { color: var(--color-success); font-size: var(--text-xs); }
...
.sticky-cta p { margin: 0; font-size: var(--text-sm); }
...
.command-panel { position: absolute; top: 12vh; left: 50%; width: min(580px, calc(100vw - var(--space-xl))); transform: translate(-50%, -8px); border: var(--rule-hair) solid var(--color-rule-2); border-radius: var(--radius-md); background: var(--color-paper); box-shadow: 0 24px 70px var(--color-shadow); overflow: hidden; transition: transform var(--dur-medium) var(--ease-out); }
...
.command-item { display: flex; justify-content: space-between; width: 100%; min-height: 46px; padding: var(--space-sm); border: 0; border-radius: var(--radius-sm); background: transparent; cursor: pointer; text-align: left; }
...
.command-panel > footer { display: flex; gap: var(--space-lg); padding: var(--space-sm) var(--space-md); border-top: var(--rule-hair) solid var(--color-rule); color: var(--color-muted); font-family: var(--font-mono); font-size: var(--text-xs); }
...
@media (max-width: 420px) {
  ...
  .verification-log div { grid-template-columns: auto minmax(0, 1fr); }
  .verification-log strong { grid-column: 2; }
