...
export const developerCodeGenPrompt = dedent`
  ...
  - When the subject calls for visual impact, use the installed creative libraries: shader backgrounds (\`@paper-design/shaders-react\`), 3D scenes (\`three\` + \`@react-three/fiber\`), post-processing (\`@react-three/postprocessing\`), particles (\`@tsparticles/react\`), or parallax (\`react-parallax\`). A creative, portfolio, gaming, music, or luxury app should feel alive — do not settle for flat color blocks when these tools are available.
