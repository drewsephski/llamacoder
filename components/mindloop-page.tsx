...
function MonochromeAvatar({ variant }: { variant: number }) {
  ...
  return (
    ...
    >
      ...
      <path
        d="M11 11.7c.8-3.4 2.5-5 5.1-5 2.9 0 4.4 1.7 4.9 4.9-1.4-.9-3.2-1.4-5.3-1.4-1.8 0-3.4.5-4.7 1.5Z"
        fill="#555"
      />
    </svg>
  );
...
function RevealWord({
  ...
  return (
    <motion.span
      className={highlighted ? "text-white" : "text-[#d9d9d9]"}
      style={{ opacity }}
    >
      {children}
    </motion.span>
    ...
    <main className="mindloop-page min-h-screen overflow-hidden bg-black font-sans text-white selection:bg-white selection:text-black">
      ...
      >
        ...
        >
          ...
          >
            Get{" "}
            <em className="mindloop-serif font-normal tracking-[-0.05em]">
              Inspired
            </em>{" "}
            with Us
          </h1>
      ...
      >
        <div className="mx-auto max-w-6xl text-center">
          <h2
            className="text-[clamp(2.75rem,7vw,7rem)] font-medium leading-[0.92] tracking-[-0.065em]"
            id="search-title"
          >
            Search has <em className="mindloop-serif font-normal">changed.</em>{" "}
            Have you?
          </h2>
          <p className="mx-auto mb-20 mt-7 max-w-2xl text-base leading-relaxed text-white/55 sm:text-lg">
            The way we find ideas is changing. The way we choose what matters
          ...
          <div className="grid gap-14 sm:grid-cols-3 sm:gap-8">
            {[
              [
                "Open Platforms",
                "Search is no longer neutral. We built an open platform for ideas worth finding.",
                Search,
              ],
              [
                "Open Knowledge",
                "Writers shape the future. We give them the tools, not the algorithms.",
                Compass,
              ],
              [
                "Open Community",
                "Communities build meaning. We connect minds, not just content.",
                Users,
              ],
      ...
      <section
        className="px-6 pb-32 pt-28 sm:px-10 md:pb-44 md:pt-36"
        id="philosophy"
      >
        <div className="mx-auto max-w-5xl">
          <motion.div
            className="relative mx-auto aspect-square max-w-[800px] overflow-hidden rounded-2xl border border-white/15"
            {...fadeUp(0)}
          >
            <video
              aria-hidden="true"
      ...
      <section
        className="border-t border-white/15 px-6 py-32 sm:px-10 md:py-44"
        id="use-cases"
      >
        <div className="mx-auto max-w-6xl">
          <motion.div {...fadeUp(0)}>
            <p className="text-xs font-medium tracking-[0.28em] text-white/45">
              SOLUTION
            </p>
            <h2 className="mt-5 max-w-4xl text-4xl font-medium leading-[0.98] tracking-[-0.055em] sm:text-6xl">
              The platform for{" "}
              <em className="mindloop-serif font-normal">meaningful</em> content
            </h2>
          </motion.div>

          <motion.div
            className="mt-14 aspect-[3/1] overflow-hidden rounded-2xl border border-white/15"
            {...fadeUp(0.1)}
          >
            <video
          ...
          <div className="mt-16 grid gap-12 md:grid-cols-4 md:gap-8">
            {features.map(({ title, description, icon: Icon }, index) => (
              ...
              >
                <Icon
                  aria-hidden="true"
                  className="mb-7 h-5 w-5 text-white/70"
                  strokeWidth={1.5}
                />
      ...
      <section
        className="relative isolate overflow-hidden border-t border-white/15 px-6 py-32 sm:px-10 md:py-44"
        id="start"
      >
        ...
        >
          ...
          <p className="mt-6 max-w-md text-base leading-relaxed text-white/55">
            Make space for the ideas that move you — and the people who
            understand why.
          </p>
          <div className="mt-10 flex flex-col items-center gap-3 sm:flex-row">
            <a
              className="inline-flex items-center justify-center rounded-lg bg-white px-8 py-3.5 text-sm font-medium text-black transition-transform hover:scale-[1.03] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/70"
              href="https://www.squidagent.app"
            >
              Build with Squid
            </a>
            <GlassButton
              className="rounded-lg px-8 py-3.5 text-sm"
              href="#use-cases"
            >
              Start Writing
            </GlassButton>
      ...
      <footer
        className="flex flex-col gap-5 px-6 py-12 text-sm text-white/45 sm:px-10 md:flex-row md:items-center md:justify-between md:px-28"
        id="footer"
      >
