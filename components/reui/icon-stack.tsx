...
function IconStack({ className, children, style, ...props }: IconStackProps) {
  return (
    <div
      ...
      className={cn(
        "**:data-[slot=icon-stack-layer]:fill-background w-18 relative h-20 text-foreground",
        className,
      )}
      style={
        {
    ...
    >
      ...
      {children ? (
        <div
          ...
          className="-skew-y-26 pointer-events-none absolute left-[var(--icon-stack-content-x)] top-[var(--icon-stack-content-y)] flex -translate-x-1/2 -translate-y-1/2 scale-x-90 items-center justify-center text-muted-foreground"
        >
          {children}
        </div>
      ) : null}
