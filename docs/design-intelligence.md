...
- **Deterministic variation** — vague briefs select a complete Style Pack from the subject bucket and brief hash without mutable filesystem state.
...
4. **Model routing** — text-only models (e.g. DeepSeek V4 Flash) auto-route to `google/gemini-3.1-flash-lite` for screenshot builds.
...
1. Add pack definitions and Hallmark theme-name mappings in `style-packs.ts`.
