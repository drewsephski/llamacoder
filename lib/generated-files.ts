...
import {
  ...
  CONDITIONAL_COLOR_ROLES,
  REQUIRED_CONTRAST_TARGETS,
  REQUIRED_SURFACE_INTERACTION_STATES,
  parseDesignSystemManifestFromCode,
...
const THEME_COLOR_CLASS_PREFIXES = new Set([
  "bg",
  "text",
  "border",
  "ring",
...
const SEMANTIC_THEME_KEYS = new Set([
  "background",
  "foreground",
  "card",
  "card-foreground",
...
const FILTER_STATE_CLASS_RE =
  /(?:^|\s)(?:[a-z0-9-]+:)*(?:hover|active|focus|focus-visible):(?:backdrop-)?(?:brightness|contrast|saturate|grayscale|sepia|invert|hue-rotate|blur|drop-shadow)-[^\s]+/;

const CONDITIONAL_SURFACE_ROLE_MARKERS = [
  {
    role: "overlay",
    reason:
      "Dialogs/alerts/menus, popovers, drawers, sheets, tooltips, or selects should be themed via an overlay role.",
    markers: [
      /\b(Dialog|DialogContent|AlertDialog|AlertDialogContent|Drawer|DrawerContent|Sheet|SheetContent|Popover|PopoverContent|Tooltip|TooltipContent|DropdownMenu|DropdownMenuContent|Select|SelectContent|ContextMenu|Menubar|NavigationMenu|HoverCard|HoverCardContent|SheetTrigger)\b/g,
    ],
  },
  {
    role: "input",
    reason:
      "Inputs (text fields, textareas, selects, checkboxes, switches) should map to an explicit input role.",
    markers: [
      /\b(<\s*input\b|<\s*textarea\b|<\s*select\b|Input\b|Textarea\b|Select\b|Checkbox\b|Switch\b|RadioGroup\b|Radio\b)\b/g,
    ],
  },
  {
    role: "table",
    reason:
      "Tabular content should use an explicit table role for consistent row/column text contrast.",
    markers: [
      /\b(<\s*table\b|<\s*thead\b|<\s*tbody\b|<\s*th\b|<\s*td\b|<\s*tr\b|Table\b|TableHeader\b|TableBody\b|TableRow\b|TableCell\b|TableHead\b)\b/g,
    ],
  },
  {
    role: "toast",
    reason:
      "Toasts and notification surfaces should have explicit toast/notification role tokens.",
    markers: [/\b(Toaster\b|Toast\b|useToast\(|\btoast\()\b/g],
  },
  {
    role: "chart",
    reason:
      "Data visuals should declare a chart role so labels/axes/tooltips can remain readable.",
    markers: [
      /\b(Chart\b|LineChart\b|BarChart\b|AreaChart\b|PieChart\b|ResponsiveContainer\b|recharts|ChartContainer\b)\b/g,
    ],
  },
];

const REQUIRED_CONDITIONALLY_SUPPORTED_ROLES = new Set(
  CONDITIONAL_COLOR_ROLES as readonly string[],
);
...
function normalizeStyleClassToken(value: string) {
  ...
  const baseClass = normalized.split(":").at(-1) ?? "";
  const baseValue = baseClass.split("/")[0]?.trim();
  if (!baseValue) return null;

  const firstDash = baseValue.indexOf("-");
  if (firstDash === -1) return null;

  const utility = baseValue.slice(0, firstDash);
  const token = baseValue.slice(firstDash + 1);
  if (!token) return null;

  return { utility, token };
}

function isTailwindStyleToken(value: string) {
  const parsed = normalizeStyleClassToken(value);

  if (!parsed) return false;

  const token = parsed.token;
  if (SEMANTIC_THEME_KEYS.has(token)) return true;
  if (token === "white" || token === "black") return true;
  if (!/^[a-z0-9-]+$/.test(token)) return false;
  if (!/^[a-z]+-\d{2,3}$/.test(token)) return false;

  return true;
}

function isInteractionStyleClassList(value: string) {
  const classes = value.split(/\s+/).filter(Boolean);

  if (classes.length === 0) return false;

  let hasSemanticColorToken = false;

  for (const className of classes) {
    if (FILTER_STATE_CLASS_RE.test(className)) {
      return false;
    }

    const parsed = normalizeStyleClassToken(className);
    if (!parsed) continue;

    if (!THEME_COLOR_CLASS_PREFIXES.has(parsed.utility)) {
      continue;
    }

    if (/^\d+$/.test(parsed.token) && parsed.utility !== "border") {
      continue;
    }

    hasSemanticColorToken = true;

    if (!isTailwindStyleToken(className)) {
      return false;
    }
  }

  return hasSemanticColorToken;
}

function collectConditionalColorRoles(appCode: string) {
  const roles = new Set<string>();

  for (const marker of CONDITIONAL_SURFACE_ROLE_MARKERS) {
    if (!REQUIRED_CONDITIONALLY_SUPPORTED_ROLES.has(marker.role)) continue;

    if (
      marker.markers.some((regex) => {
        regex.lastIndex = 0;
...
function formatConditionalRoleRequirementHints(required: string[]) {
  const markerMap = new Map(
    CONDITIONAL_SURFACE_ROLE_MARKERS.map((entry) => [entry.role, entry]),
  );

  return required
    .map((role) => {
      const marker = markerMap.get(role);
      const reason =
        marker?.reason ??
        "Use an explicit manifest role for this surface type.";
...
function validateDesignSystemManifest(files: GeneratedFile[]) {
  ...
  if (!manifest) {
    diagnostics.push({
      path: appFile.path,
      message:
        "Missing or malformed __designSystemManifest constant. Define a JSON parseable manifest object in App.tsx before export.",
    });

    return diagnostics;
  }

  if (!isObject(manifest.colorRoles)) {
    diagnostics.push({
      path: appFile.path,
      message:
        "__designSystemManifest.colorRoles must be an object of named surface roles with background/foreground classes.",
    });

    return diagnostics;
  }

  const roleEntries = Object.entries(manifest.colorRoles);
  const missingRoles = REQUIRED_COLOR_ROLES.filter(
    (role) => !manifest.colorRoles || !(role in manifest.colorRoles),
  );

  const conditionalRoles = collectConditionalColorRoles(appFile.code);
  const missingConditionalRoles = conditionalRoles.filter(
    (role) => !manifest.colorRoles || !(role in manifest.colorRoles),
  );

  if (
    roleEntries.length < REQUIRED_COLOR_ROLES.length ||
    missingRoles.length > 0
  ) {
    diagnostics.push({
      path: appFile.path,
      message: `__designSystemManifest.colorRoles is incomplete. Include at least these keys: ${REQUIRED_COLOR_ROLES.join(", ")}.`,
    });
  }

  if (missingConditionalRoles.length > 0) {
    diagnostics.push({
      path: appFile.path,
      message: `__designSystemManifest.colorRoles is missing conditional roles for rendered surface types: ${formatConditionalRoleRequirementHints(missingConditionalRoles)}.`,
    });
  }
  ...
  for (const [roleName, roleValue] of roleEntries) {
    if (!isObject(roleValue)) {
      diagnostics.push({
        path: appFile.path,
        message: `__designSystemManifest color role "${roleName}" must be an object with background and foreground fields.`,
      });
      continue;
    }

    const { background, foreground, border, contrastTargets } = roleValue;
    if (typeof background !== "string" || typeof foreground !== "string") {
      diagnostics.push({
        path: appFile.path,
        message: `__designSystemManifest color role "${roleName}" must define both background and foreground class names.`,
      });
      continue;
    }

    if (
      !isTailwindStyleToken(background) ||
      !isTailwindStyleToken(foreground)
    ) {
      diagnostics.push({
        path: appFile.path,
        message: `__designSystemManifest color role "${roleName}" uses unsupported token format: background="${background}", foreground="${foreground}".`,
      });
    }

    if (
      border !== undefined &&
      (typeof border !== "string" || !isTailwindStyleToken(border))
    ) {
      diagnostics.push({
        path: appFile.path,
        message: `__designSystemManifest color role "${roleName}" has an unsupported border class "${border}".`,
      });
    }

    if (contrastTargets !== undefined) {
      if (!isObject(contrastTargets)) {
        diagnostics.push({
          path: appFile.path,
          message: `__designSystemManifest color role "${roleName}" contrastTargets must be an object with numeric values.`,
        });
      } else {
        for (const [targetName, targetValue] of Object.entries(
          contrastTargets as Record<string, unknown>,
        )) {
          if (!isPositiveFiniteNumber(targetValue)) {
            diagnostics.push({
              path: appFile.path,
              message: `__designSystemManifest color role "${roleName}" contrastTargets.${targetName} must be a finite numeric value.`,
            });
          }
        }
      }
  ...
  if (
    manifest.contrastTargets === undefined ||
    !isObject(manifest.contrastTargets)
  ) {
    diagnostics.push({
      path: appFile.path,
      message:
        "__designSystemManifest.contrastTargets must be an object with bodyText, largeText, interactive, focusRing, and disabled numeric targets.",
    });
  } else {
    for (const [targetName, requiredTarget] of Object.entries(
      REQUIRED_CONTRAST_TARGETS,
    )) {
      const targetValue = (manifest.contrastTargets as Record<string, unknown>)[
        targetName
      ];
      if (!isPositiveFiniteNumber(targetValue)) {
        diagnostics.push({
          path: appFile.path,
          message: `__designSystemManifest.contrastTargets.${targetName} must be a numeric value.`,
        });
      } else if (targetValue < requiredTarget) {
        diagnostics.push({
          path: appFile.path,
          message: `__designSystemManifest.contrastTargets.${targetName} must be at least ${requiredTarget}.`,
        });
      }
    }
  }
  ...
  if (
    manifest.surfaceInteractions === undefined ||
    !isObject(manifest.surfaceInteractions)
  ) {
    diagnostics.push({
      path: appFile.path,
      message:
        "__designSystemManifest.surfaceInteractions must be an object with required interaction state classes.",
    });
    return diagnostics;
  }

  const missingStates = REQUIRED_SURFACE_INTERACTION_STATES.filter(
    (state) => !(state in manifest.surfaceInteractions!),
  );
  if (missingStates.length > 0) {
    diagnostics.push({
      path: appFile.path,
      message: `__designSystemManifest.surfaceInteractions is incomplete. Include at least: ${missingStates.join(", ")}.`,
    });
  }

  if (
    !("focus" in manifest.surfaceInteractions) &&
    !("focus-visible" in manifest.surfaceInteractions)
  ) {
    diagnostics.push({
      path: appFile.path,
      message:
        "__designSystemManifest.surfaceInteractions must include either `focus` or `focus-visible` state classes.",
    });
  }

  for (const [stateName, stateTokens] of Object.entries(
    manifest.surfaceInteractions,
  )) {
    if (typeof stateTokens !== "string") {
      diagnostics.push({
        path: appFile.path,
        message: `__designSystemManifest.surfaceInteractions.${stateName} must be a string class list.`,
      });
      continue;
    }
    if (!isInteractionStyleClassList(stateTokens)) {
      diagnostics.push({
        path: appFile.path,
        message: `__designSystemManifest.surfaceInteractions.${stateName} must contain valid color-aware classes, for example hover:bg-*/focus-visible:ring-* / text-* or selected/success/error tokens with theme-safe values.`,
      });
    }
  }
...
export function validateGeneratedFiles(
  ...
  diagnostics.push(...validateDesignSystemManifest(files));
...
${generatedAppRepairCapabilityRules}
      - If \`__designSystemManifest\` is missing or invalid, emit a JSON-parseable manifest object in App.tsx with at least these required roles: \`canvas\`, \`surface\`, \`mutedSurface\`, \`inverse\`, \`primary\`, \`secondary\`, \`accent\`, \`success\`, \`destructive\`, plus \`contrastTargets\` and required interaction states. If the app uses dialogs/menus/drawers/overlays, inputs, tables, toasts, or charts, include matching conditional roles (\`overlay\`, \`input\`, \`table\`, \`toast\`, \`chart\`) too.
